...
class _ApplicationsScreenState extends ConsumerState<ApplicationsScreen> {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => SingleChildScrollView(
          padding: EdgeInsets.fromLTRB(20.w, 16.h, 20.w, MediaQuery.of(ctx).viewInsets.bottom + 28.h),
          child: Column(
            ...
            children: [
              ...
              // Section statuts
              Text(t.t('applications_screen.filter_status'),
                  style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w700, color: cs.onSurface)),
              SizedBox(height: 12.h),
              Wrap(spacing: 8.w, runSpacing: 8.h, children: ApplicationStatuses.all.map((cfg) {
                final sel = tempStatuses.contains(cfg.key);
                return GestureDetector(
                  onTap: () => setSheet(() => sel ? tempStatuses.remove(cfg.key) : tempStatuses.add(cfg.key)),
                  child: AnimatedContainer(
                    duration: const Duration(milliseconds: 150),
                    padding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 7.h),
                    decoration: BoxDecoration(
                      color: sel ? cfg.bgColor : cs.surfaceContainerHighest.withValues(alpha: 0.4),
                      borderRadius: BorderRadius.circular(16.r),
                      border: Border.all(color: sel ? cfg.borderColor : cs.outlineVariant, width: 1.5),
                    ),
                    child: Text(t.t('applications_screen.status_${cfg.key}'),
              ...
              // Section date
              Text(t.t('applications_screen.filter_date'),
                  style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w700, color: cs.onSurface)),
              SizedBox(height: 12.h),
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(16.r),
                      border: Border.all(color: sel ? cs.primary : cs.outlineVariant, width: 1.5),
                    ),
                    child: Text(e.$2, style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.w600,
              ...
              SizedBox(width: double.infinity, child: FilledButton(
                onPressed: () {
                  setState(() { _selectedStatuses..clear()..addAll(tempStatuses); _timeFilter = tempTime; });
                  Navigator.pop(ctx);
                },
                style: FilledButton.styleFrom(minimumSize: Size(0, 48.h),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.r))),
                child: Text(t.t('applications_screen.filter_apply'),
  ...
  Future<void> _showAdd(AppLocalizations t) async {
    final result = await showDialog<AddApplicationResult>(context: context, builder: (_) => const AddApplicationDialog());
    if (result == null || !mounted) return;
    try {
      await ref.read(applicationsProvider.notifier).create(result.data, cvPath: result.cvPath, cvFilename: result.cvFilename);
    } catch (_) {
    ...
    return Scaffold(
      floatingActionButton: Container(
        ...
        child: FloatingActionButton(
          onPressed: () => _showAdd(t), elevation: 0,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.r)),
      ...
      body: SafeArea(
        child: appsAsync.when(
          ...
          error: (_, _) => Center(child: Text(t.t('applications_screen.add_error'))),
          data: (apps) {
            final filtered = _filter(apps);
            return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              // En-tête
              Padding(padding: EdgeInsets.fromLTRB(20.w, 16.h, 20.w, 0), child: Column(
                crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(t.t('applications_screen.title'),
                      style: TextStyle(fontSize: 22.sp, fontWeight: FontWeight.w800, color: cs.onSurface)),
                  SizedBox(height: 4.h),
                  Text('${filtered.length} ${t.t('applications_screen.results_count')}',
              ...
              if (_hasFilters) ...[SizedBox(height: 10.h), _buildActiveChips(t, cs)],
              SizedBox(height: 12.h),
              // Liste ou état vide
              Expanded(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => Padding(
                            ...
                            child: ApplicationCard(
                              app: filtered[i],
                              onTap: () => context.push('/applications/${filtered[i]['id']}'),
                              onDelete: () => _confirmDelete(filtered[i]['id'].toString(), t),
                            ),
                          ),
                        ),
                      ),
  ...
  Widget _buildSearchBar(AppLocalizations t, ColorScheme cs) {
    return Padding(padding: EdgeInsets.symmetric(horizontal: 20.w), child: Row(children: [
      Expanded(child: Container(
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(12.r),
            border: Border.all(color: const Color(0xFFD1D5DB), width: 1.5)),
        child: TextField(controller: _searchController, style: TextStyle(fontSize: 14.sp),
          decoration: InputDecoration(
            ...
            border: InputBorder.none, contentPadding: EdgeInsets.symmetric(vertical: 12.h),
          )),
      )),
      SizedBox(width: 10.w),
      GestureDetector(onTap: () => _openFilters(t), child: AnimatedContainer(
        duration: const Duration(milliseconds: 150), width: 46.w, height: 46.h,
        decoration: BoxDecoration(
          color: _hasFilters ? cs.primary : Colors.transparent,
          borderRadius: BorderRadius.circular(12.r),
          border: Border.all(color: _hasFilters ? cs.primary : const Color(0xFFD1D5DB), width: 1.5),
        ),
        child: Icon(Icons.tune_rounded, size: 20.sp, color: _hasFilters ? cs.onPrimary : cs.onSurfaceVariant),
      )),
    ]));
  }

  Widget _buildActiveChips(AppLocalizations t, ColorScheme cs) {
    return SizedBox(height: 34.h, child: ListView(
      scrollDirection: Axis.horizontal, padding: EdgeInsets.symmetric(horizontal: 20.w),
      children: [
        ..._selectedStatuses.map((key) {
          final cfg = ApplicationStatuses.fromKey(key);
          return Padding(padding: EdgeInsets.only(right: 8.w), child: DismissibleFilterChip(
            label: t.t('applications_screen.status_$key'),
            textColor: cfg.textColor, bgColor: cfg.bgColor, borderColor: cfg.borderColor,
            onRemove: () => setState(() => _selectedStatuses.remove(key)),
          ));
        }),
        if (_timeFilter != 'all')
          DismissibleFilterChip(label: _timeLabel(t), textColor: cs.primary,
            bgColor: cs.primary.withValues(alpha: 0.1), borderColor: cs.primary.withValues(alpha: 0.4),
  ...
  Widget _buildEmpty(AppLocalizations t, ColorScheme cs, bool globalEmpty) {
    return Center(child: Padding(padding: EdgeInsets.symmetric(horizontal: 40.w), child: Column(
      mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(width: 80.r, height: 80.r,
          decoration: BoxDecoration(color: cs.primary.withValues(alpha: 0.1), shape: BoxShape.circle),
          child: Icon(globalEmpty ? Icons.work_outline_rounded : Icons.search_off_rounded, size: 36.sp, color: cs.primary)),
        SizedBox(height: 20.h),
        Text(globalEmpty ? t.t('applications_screen.empty_title') : t.t('applications_screen.no_results'),
            style: TextStyle(fontSize: 17.sp, fontWeight: FontWeight.w700, color: cs.onSurface), textAlign: TextAlign.center),
        if (globalEmpty) ...[
          SizedBox(height: 8.h),
          Text(t.t('applications_screen.empty_subtitle'),
              style: TextStyle(fontSize: 14.sp, color: cs.onSurfaceVariant), textAlign: TextAlign.center),
          SizedBox(height: 24.h),
          FilledButton.icon(onPressed: () => _showAdd(t), icon: Icon(Icons.add_rounded, size: 18.sp),
            label: Text(t.t('applications_screen.empty_add')),
            style: FilledButton.styleFrom(minimumSize: Size(0, 44.h),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.r)))),
        ],
      ],
    )));
