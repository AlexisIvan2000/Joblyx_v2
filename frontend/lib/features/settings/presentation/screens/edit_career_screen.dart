...
class _EditCareerScreenState extends ConsumerState<EditCareerScreen> {
  ...
  Future<void> _uploadCv() async {
    ...
    try {
      final extracted = await ref.read(roadmapServiceProvider).extractSkills(result.files.first.path!);
      if (!mounted) return;
      int added = 0;
      for (final s in extracted) {
        ...
        if (!_selectedSkills.any((c) => c.skillName == name)) {
          _selectedSkills.add(SkillChipData(
            skillName: name, category: s['category'] as String,
            proficiency: s['proficiency'] as String? ?? 'intermediate',
          ));
          added++;
        }
      ...
      setState(() {});
      if (mounted) AppSnackbar.success(context, t.t('onboarding.skills_extracted').replaceAll('{count}', '$added'));
    } catch (_) {
