...
from api.v1.client.dependencies import get_auth_service, get_user_service, get_current_user, get_r2_service
...
async def upload_avatar(
    ...
    # Supprime l'ancien avatar avant d'uploader le nouveau
    if current_user.avatar_url:
        try:
            await r2.delete_avatar(current_user.avatar_url)
        except Exception:
            pass
    ...
    # On persiste le file_key, pas l'URL signée qui expire
...
async def delete_account(
    ...
    return await svc.delete_account(
        str(current_user.id), email,
        r2_service=r2,
